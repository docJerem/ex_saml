...
- **A REST API over the debug tooling** (`ExSaml.DebugRouter`): support and on-call can diagnose a failed sign-in without a remote console — read the activation state, turn debug on for one IdP, list its recent failures, read a trace, download the captured `SAMLResponse` as base64 or XML, replay it against the live configuration, turn debug off. A `Plug.Router` mounted behind the consumer's own admin pipeline (`forward "/", ExSaml.DebugRouter, authorize: {Mod, :fun}`), it performs no authentication itself but refuses to start without an explicit `:authorize` decision, which may return `{:ok, [idp_id]}` to scope every route to one tenant. Naming an IdP outside that set is 403; a `trace_id` outside it is 404, indistinguishable from one that expired, because trace ids travel — they are the `?error_id=` a user pastes into a support ticket. A replay whose response still fails answers 200 `"result": "error"` (the ordinary answer to "is it fixed yet?"); only a replay that could not be attempted is 4xx. `?redact=false` is off by default and separate from mounting, since an unredacted trace carries the authorization code, a live bearer credential. Writes with no cache configured are refused with 409 rather than arming a single node. One audit line per request, marked `pii=true` on payload downloads and unredacted traces. Options may also be set under `config :ex_saml, debug_api:`. Guide: `guides/debug_api.md` (#59)
- `ExSaml.Debug.scope_status/1` (one scope's activation state without `status/0`'s full cache scan), `ExSaml.Debug.config/0` (the retention settings, so nothing has to restate the defaults) and `ExSaml.Debug.redact_trace/1` (documented: `redact/1` takes a single map and silently passes a whole trace through unredacted) (#59)
- `ExSaml.Debug.JSON`: serialises traces and captures — atoms, tuples, charlists, xmerl records, `DateTime`s, structs, non-UTF-8 binaries — with no mandatory JSON dependency. The encoder is part of the library (OTP's `:json` starts at OTP 27, and it renders Elixir's `nil` as the string `"nil"`); decoding uses Jason if present, `:json` otherwise, or `config :ex_saml, debug_api: [json_decoder: {Mod, :fun}]`. `{:jason, "~> 1.0", optional: true}` is now declared (#59)
- The guides are published with the docs at last: `mix.exs` had no `docs/0`, so `guides/` shipped nowhere (#59)

- **Response validation checks** (first half of #55, re-integrated from #58): `Response/Issuer` and `Assertion/Issuer` against the IdP `entityID` (`:bad_issuer`, Core §2.2.3 / Profiles §4.1.4.2), `SubjectConfirmationData/@InResponseTo` against the ID of the AuthnRequest the SP sent (`:bad_in_response_to`, Profiles §4.1.4.3), `AuthnStatement/@SessionNotOnOrAfter` (`:session_expired`, Core §2.7.2) and a bearer `SubjectConfirmation/@Method` (`:bad_subject_confirmation`, Profiles §4.1.4.2). `config :ex_saml, :enforced_response_checks` lists the checks that reject (default: these four; `[]` downgrades everything to log-only, `:all` enforces every known check); a non-enforced failure emits one grep-able `[ExSaml] saml_validation check=… enforced=false expected=… actual=…` line and a `:validation_check` event in the debug trace. `:bad_destination` and `:duplicate` are catalogued but not evaluated yet. New `ExSaml.Core.ValidationContext` struct carries what the checks need (audience, recipient, destination, IdP entityID and id, the enforce list and the evaluation instant `now`); `ExSaml.Core.SpConfig` gains `idp_entity_id` / `idp_id` (filled from `%ExSaml.IdpData{}`, nothing new for consumers) and `SpConfig.entity_id/1`; `ExSaml.Core.Sp.xml_id/1` and `ExSaml.Helper.authn_request_id/1` read back the ID minted in the AuthnRequest, which `ExSaml.AuthHandler` stores in the relay state and the session. Deliberate failure modes: a missing stored AuthnRequest ID skips the check (rolling deploys, consumers driving `validate_authresp/4` themselves), an unparsable `SessionNotOnOrAfter` warns and passes, an absent `Response/Issuer` is silent, the response-level checks run after signature verification. Messages in English and French (#55, #58)
...
- `ExSaml.Debug.replay/2` no longer lets a decode raise or throw through its caller: a capture holds whatever the IdP posted, xmerl throws on malformed XML, and this was the one public debug function with no `rescue`. It now returns `{:error, %ExSaml.Error{reason: :exception, step: :decode}}`, matching what `consume_signin_response/2` has always done for the live flow. It also reached into captures with dot access, so a flow stashed without `saml_encoding` — captures are built with `Map.take/2`, absent rather than nil — raised a `KeyError` (#59)
- `ExSaml.Debug.replay/2`'s `@spec` said `ExSaml.Core.Assertion.t()`; it returns `ExSaml.Assertion.t()`, the public struct with stringified attributes (#59)
...
- `ExSaml.Core.Saml.validate_assertion/2` takes an `ExSaml.Core.ValidationContext`; the `/3` and `/4` (recipient, audience, `now:`) forms remain and delegate with a minimal context, skipping the checks that need one. `ExSaml.Core.Sp.validate_assertion/3,4` accept a `now:` option (additive) and build the context from the `SpConfig` (#54, #55)
